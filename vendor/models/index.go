...
func GetDashboard() []Dashboard {
	...
	dashboard := []Dashboard{}
	db.Select(&dashboard, fmt.Sprintf("call sp_getDashboard('%s','%s')", start_time.UTC().Format(DatetimeFormat), end_time.UTC().Format(DatetimeFormat)))
...
func GetTopOutRate() []Top {
	db := GetConnection()
	top := []Top{}
	db.Select(&top, fmt.Sprintf("call sp_getTopOutRate"))
	return top
}
...
func GetTopWinRate() []Top {
	db := GetConnection()
	top := []Top{}
	db.Select(&top, fmt.Sprintf("call sp_getTopWinRate"))
	return top
}
...
func GetTopHitRate() []Top {
	db := GetConnection()
	top := []Top{}
	db.Select(&top, fmt.Sprintf("call sp_getTopHitRate"))
	return top
}
