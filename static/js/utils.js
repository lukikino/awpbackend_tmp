...
Utils.url = (function () {
    return {
        getHrefAndReplace: function (searchs, hash, source) {
            var _location = document.createElement('a');
            _location.href = (source || location.href);
            var search = _location.search.replace('?', '') || '';
            var searchResult = [];
            var searchList = {};
            var oriSearchList = search.split('&');
            hash = (hash === undefined || hash === null) ? _location.hash : hash;
            if (searchs) {
                for (var _searchItemIndex in oriSearchList) {
                    var searchItemList = (oriSearchList[_searchItemIndex] && typeof oriSearchList[_searchItemIndex] == "string") ? oriSearchList[_searchItemIndex].split('=') : [];
                    if (searchItemList && searchItemList.length > 1) {
                        searchList[searchItemList[0]] = searchItemList[1];
                    }
                }
                for (var _search in searchs) {
                    searchList[_search] = searchs[_search];
                }
                for (var _search in searchList) {
                    if (searchList[_search] && typeof searchList[_search] != 'object' && typeof searchList[_search] != 'function') {
                        searchResult.push(_search + '=' + searchList[_search]);
                    }
                }
                //return _location.protocol + "//" + _location.host + _location.pathname + "?" + searchResult.join('&') + hash;
                var pathname = _location.pathname;
                if (_location.pathname.length && _location.pathname[0] != '/') {
                    pathname = '/' + _location.pathname;
                }
                // return _location.protocol + "//" + _location.host + pathname + "?" + searchResult.join('&') + hash;
                return _location.protocol + "//" + _location.hostname + ((_location.port) ? ":" + _location.port : "") + pathname + (searchResult.length ? "?" : "") + searchResult.join('&') + hash;
            }
            else {
                return _location.href;
            }
        },
        getSearchValue: function (key, source) {
            var _location = document.createElement('a');
            _location.href = (source || location.href);
            var search = _location.search.replace('?', '') || '';
            var oriSearchList = search.split('&');
            if (search) {
                for (var _searchItemIndex in oriSearchList) {
                    var searchItemList = (oriSearchList[_searchItemIndex] && typeof oriSearchList[_searchItemIndex] == "string") ? oriSearchList[_searchItemIndex].split('=') : [];
                    if (searchItemList && searchItemList.length > 1 && searchItemList[0].toLowerCase() == key.toLowerCase()) {
                        return searchItemList[1];
                    }
                }
                return "";
            }
...
Utils.date = (function () {
    var o = {};
    o.getDayStart = function (date) {
        return new Date(date).toString("yyyy/MM/dd") + " 6:00:00";
    };
    o.getDayEnd = function (date) {
        return new Date(date).addDays(1).toString("yyyy/MM/dd") + " 5:59:59";
    };
    o.monthStart = function () {
        var now = (new Date);
        if (now.getHours() < 6) {
    ...
    o.yesterdayStart = function () {
        return o.todayStart().addDays(-1);
    };
    o.yesterdayEnd = function () {
        return o.todayEnd().addDays(-1);
    };
    o.todayStart = function () {
        var now = (new Date);
        if (now.getHours() < 6) {
            return now.clearTime().addDays(-1).addHours(6);
        } else {
            return now.clearTime().addHours(6);
        }
    };
    o.todayEnd = function () {
        var now = (new Date);
        if (now.getHours() < 6) {
            return now.clearTime().addHours(6).addSeconds(-1);
        } else {
            return now.clearTime().addDays(1).addHours(6).addSeconds(-1);
        }
    };
    return o;
})();
...
Utils.cookie = function () {
    return {
        ...
        cookieErase: function (name) {
            FDO.common.cookieCreate(name, "", -1);
        }
    }
}();
